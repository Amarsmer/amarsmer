...
def generate_launch_description():
    ...
    with sl.group(if_arg='gui'):
        sl.gz_launch(sl.find('amarsmer_description', 'demo_world.sdf'), "-r")
        
    with sl.group(unless_arg='gui'):
        sl.gz_launch(sl.find('amarsmer_description', 'demo_world.sdf'), "-r -s")
    ...
    with sl.group(if_arg='spawn'):
        sl.include('amarsmer_description', 'upload_bluerov2_launch.py')
